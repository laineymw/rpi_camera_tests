...
# make arrowkeys work
# maybe make example autofocus or something
# add camera

import sys, time, serial, RPi.GPIO as GPIO
...
class CNCWorker(QThread):
    ...
    def run(self):
        if self.command:
            self.cnc.move_XYZ(self.command)
...
class ModernMainWindow(QMainWindow):
    ...
    def __init__(self):
        ...
        arrow_layout.addWidget(self.forward_button, 0, 2)
        arrow_layout.addWidget(self.right_button, 1, 2)
        arrow_layout.addWidget(self.down_button, 1, 1)
        arrow_layout.addWidget(self.backwards_button, 0, 0)
        arrow_layout.addWidget(self.left_button, 1, 0)
       
        main_layout.addWidget(arrow_frame)

        # Initialize stream redirector
        sys.stdout = StreamRedirector(self.log_text)
        sys.stderr = StreamRedirector(self.log_text)
    ...
    def jog(self, axis, direction):
        ...
        self.cnc_thread = CNCWorker(self.cnc)
        self.cnc_thread.command = pos
    ...
    def safe_home(self):
        ...
        self.cnc.home_grbl()
        pos = self.cnc.get_current_position()
        self.print_with_timestamp(f"Moved To: {pos}")
